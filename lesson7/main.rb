    ...
    if @passenger_trains.empty?
      ...
      if @passenger_trains[train_number_choice_for_take_seat].wagons.empty?
        ...
        @passenger_trains[train_number_choice_for_take_seat].wagons.each_with_index do |wagon, index|
          puts "#{index + 1}. Вагон с количеством мест - #{wagon.total_seats}"
        end
        wagon_choice = gets.chomp.to_i - 1

        if @passenger_trains[train_number_choice_for_take_seat].wagons[wagon_choice].taken_seats < @passenger_trains[train_number_choice_for_take_seat].wagons[wagon_choice].total_seats
          @passenger_trains[train_number_choice_for_take_seat].wagons[wagon_choice].take_seat
          puts "Место занято. Всего занятых мест в вагоне - #{@passenger_trains[train_number_choice_for_take_seat].wagons[wagon_choice].taken_seats} . Свободных мест в вагоне осталось - #{@passenger_trains[train_number_choice_for_take_seat].wagons[wagon_choice].free_seats}"
        else
    ...
    if @cargo_trains.empty?
      ...
      if @cargo_trains[train_number_choice_for_take_volume].wagons.empty?
        ...
        @cargo_trains[train_number_choice_for_take_volume].wagons.each_with_index do |wagon, index|
          puts "#{index + 1}. Вагон с объемом - #{wagon.total_volume}"
        end
        wagon_choice = gets.chomp.to_i - 1

        puts "Укажите объем который хотите занять. Доступный объем у вагона - #{@cargo_trains[train_number_choice_for_take_volume].wagons[wagon_choice].free_volume}"
        volume_choice = gets.chomp.to_i

        if volume_choice <= @cargo_trains[train_number_choice_for_take_volume].wagons[wagon_choice].free_volume
          @cargo_trains[train_number_choice_for_take_volume].wagons[wagon_choice].fill_volume(volume_choice)
          puts "Заданный объем занят. Общий занятый объем вагона - #{@cargo_trains[train_number_choice_for_take_volume].wagons[wagon_choice].filled_volume}. Доступный объем - #{@cargo_trains[train_number_choice_for_take_volume].wagons[wagon_choice].free_volume}"
        else
    ...
    when 1
      if @passenger_trains.empty?
        ...
        if @passenger_trains[train_number_choice].wagons.empty?
          ...
          wagon_number = 0
          @passenger_trains[train_number_choice].each_wagons {|wagon| puts "#{wagon_number += 1}. Тип вагона: #{wagon.type}. Количество мест - #{wagon.total_seats}. Количество свободных мест - #{wagon.free_seats}"}
        end
      end
    when 2
      if @cargo_trains.empty?
        ...
        if @cargo_trains[train_number_choice].wagons.empty?
          ...
          wagon_number = 0
          @cargo_trains[train_number_choice].each_wagons {|wagon| puts "#{wagon_number += 1}. Тип вагона: #{wagon.type}. Объем вагона - #{wagon.total_volume}. Доступный объем - #{wagon.free_volume}"}
        end
      end
    else
      puts "Выберите цифру, которая соответствует пунктам от 1 до 2"
